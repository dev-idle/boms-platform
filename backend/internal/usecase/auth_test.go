...
func TestAuthUsecase_RegisterDuplicateEmail(t *testing.T) {
	...
	uc := newAuthUC(t, users, sessions, hasher, signer)
...
func TestAuthUsecase_RefreshReuseJTImismatch(t *testing.T) {
	...
	sessions := new(mockSessionStore)
	signer := new(mockSigner)
	uc := newAuthUC(t, users, sessions, new(mockHasher), signer)
...
func TestAuthUsecase_RefreshMissingSession(t *testing.T) {
	...
	sessions := new(mockSessionStore)
	signer := new(mockSigner)
	uc := newAuthUC(t, new(mockUserRepo), sessions, new(mockHasher), signer)
...
func TestAuthUsecase_RefreshAccessTokenRejected(t *testing.T) {
	t.Parallel()
	signer := new(mockSigner)
	uc := newAuthUC(t, new(mockUserRepo), new(mockSessionStore), new(mockHasher), signer)
...
func TestAuthUsecase_LogoutIdempotent(t *testing.T) {
	t.Parallel()
	sessions := new(mockSessionStore)
	uc := newAuthUC(t, new(mockUserRepo), sessions, new(mockHasher), new(mockSigner))
...
func TestAuthUsecase_LogoutHybridNone(t *testing.T) {
	t.Parallel()
	uc := newAuthUC(t, new(mockUserRepo), new(mockSessionStore), new(mockHasher), new(mockSigner))
