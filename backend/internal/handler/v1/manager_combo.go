...
type ManagerComboHandler struct {
	usecase *usecase.ManagerComboUsecase
}

func NewManagerComboHandler(uc *usecase.ManagerComboUsecase) *ManagerComboHandler {
	return &ManagerComboHandler{usecase: uc}
}

func (h *ManagerComboHandler) List(c *fiber.Ctx) error {
...
func (h *ManagerComboHandler) Get(c *fiber.Ctx) error {
	response.EnsureRequestID(c)
...
func (h *ManagerComboHandler) Create(c *fiber.Ctx) error {
	response.EnsureRequestID(c)
...
func (h *ManagerComboHandler) Patch(c *fiber.Ctx) error {
	response.EnsureRequestID(c)
	actorID, actorRole, err := actorFromCtx(c)
	if err != nil {
		return err
...
func (h *ManagerComboHandler) Delete(c *fiber.Ctx) error {
	response.EnsureRequestID(c)
	actorID, actorRole, err := actorFromCtx(c)
	if err != nil {
		return err
